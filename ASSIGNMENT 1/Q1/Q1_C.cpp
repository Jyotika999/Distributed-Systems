...
int main()
{
	...
	#pragma omp parallel for shared(matrix1,matrix2,product)
    for(long long int i=0;i<1024;i++)
    {
        for(long long int j=0;j<1024;j++)
        {
    ...
    cout<<  "TOTAL TIME TAKEN BY PARALLELIZATION BY OPENMP PROGRAM TO RUN IS "<< fixed  << time_taken << setprecision(9);
    cout << " sec" << endl;
